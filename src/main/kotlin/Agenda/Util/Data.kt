...
import Agenda.Util.SimpleTime.Companion.decode
import java.util.*
...
data class Day(val name: Weekday, val date: SimpleDate, val events: Array<Event>) {
    override fun toString() = "Day (${name.dayname} $date: ${Arrays.deepToString(events)})"
...
data class Event(val name: String, val short: String, val location: String, val start: SimpleTime, val end: SimpleTime) {
    ...
    fun getDataContent() = "events/event-${name.toLowerCase(Locale.ENGLISH).replace(' ', '-')}"
}
...
data class SimpleTime(val hours: Int, val minutes: Int) {
    companion object {
        ...
        @JvmStatic private val regex = Regex("(\\d+):(\\d{2})")
         ...
         */
        @JvmStatic fun decode(time: String): Pair<Int, Int> {
